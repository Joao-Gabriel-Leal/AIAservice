<x-layouts.portal title="Usuários">
    <div class="space-y-6">
        <div class="flex justify-end">
            <a href="{{ route('users.create') }}" class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white">Novo usuário</a>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50 text-left text-slate-500">
                    <tr>
                        ...
                        <th class="px-6 py-3 font-medium">Setor / Sala</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($users as $listedUser)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-slate-600">{{ $listedUser->role->label() }}</td>
                            <td class="px-6 py-4 text-slate-600">
                                <p>{{ $listedUser->sector?->name ?? 'Sem setor' }}</p>
                                <p class="text-xs text-slate-500">{{ $listedUser->room?->name ?? 'Sem sala' }}</p>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    @can('delete', $listedUser)
                                        <form method="POST" action="{{ route('users.destroy', $listedUser) }}" onsubmit="return confirm('Remover usuário?');">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-slate-500">Nenhum usuário cadastrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
