<x-layouts.portal :title="$asset->name" subtitle="Detalhe completo do patrimonio, localizacao atual, QR Code e historico cronologico." header-variant="none">
    <div class="space-y-6">
        <x-portal.page-intro
            variant="detail"
            :eyebrow="$asset->currentSector?->company?->name ?? 'Patrimonio operacional'"
            :title="$asset->name"
            :description="$asset->description ?: 'Detalhe completo do patrimonio com localizacao atual, QR Code e historico cronologico.'"
        >
            <x-slot:actions>
                @can('update', $asset)
                    <a href="{{ route('assets.edit', $asset) }}" class="ui-action ui-action-secondary rounded-2xl px-4 py-3 text-sm">Editar cadastro</a>
                @endcan
                @can('move', $asset)
                    <a href="{{ route('assets.movement.create', $asset) }}" class="ui-action ui-action-primary rounded-2xl px-4 py-3 text-sm">Movimentar</a>
                @endcan
            </x-slot:actions>
            <x-slot:meta>
                <span class="portal-chip">{{ $asset->asset_code }}</span>
                @if ($asset->currentSector)
                ...
                <span class="portal-chip">{{ $asset->operationalStateLabel() }}</span>
            </x-slot:meta>
        </x-portal.page-intro>
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px]">
                <div class="space-y-6">
                    <div class="border-b border-slate-200 pb-4">
                        <div>
                            <h2 class="text-lg font-semibold text-slate-900">Local atual e dados rapidos</h2>
                            <p class="mt-1 text-sm text-slate-500">Visao curta do ativo para validar setor, sala, colaborador e origem antes de abrir o historico.</p>
                        </div>
                    </div>
