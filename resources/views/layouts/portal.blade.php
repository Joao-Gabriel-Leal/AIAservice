            ...
            <aside class="border-b border-slate-800 bg-slate-950 px-6 py-6 text-slate-100 lg:border-b-0 lg:border-r">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-400">Gestão interna modular</p>
                    </div>
                </a>
                ...
                <nav class="mt-8 space-y-8">
                    ...
                    @if (auth()->user()->isSuperAdmin() || auth()->user()->isSectorAdmin())
                        <div>
                            <p class="mb-3 text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Administração</p>
                            <div class="space-y-2">
                                ...
                                <a href="{{ route('users.index') }}" class="{{ request()->routeIs('users.*') ? 'bg-sky-500/20 text-sky-200' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }} block rounded-xl px-4 py-3 text-sm transition">Usuários</a>
